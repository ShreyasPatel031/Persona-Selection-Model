#!/usr/bin/env bash
# SSV fine K sweep @ L15 on gemma-dsweep-good
set -euo pipefail
cd "$HOME/gemma-chat"
...
LOG="logs/ssv_k_sweep_l15_20q.log"

exec > >(tee -a "$LOG") 2>&1
echo "=== SSV K sweep L15 $(date -Is) ==="
.venv/bin/python3 -u scripts/ssv_omp_k_sweep.py \
  --trait good --method ssv \
  --ks "$KS" --n-questions 20 --judge-workers 10 --resume \
  --out persona_runs/dnd_good_scale/sae/ssv_k_sweep_l15_20q.json
echo "=== DONE $(date -Is) ==="
