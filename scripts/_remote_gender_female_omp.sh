#!/usr/bin/env bash
# OMP bubble viz pipeline for gender_female @ L15, K sweep through K=20 only.
set -euo pipefail
...
LOG="$LOG_DIR/gender_female_omp_${TS}.log"
exec > >(tee -a "$LOG") 2>&1

echo "=== gender_female OMP pipeline started $(date -Is) ==="
