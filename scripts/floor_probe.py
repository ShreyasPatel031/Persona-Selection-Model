...
and the rotations differ in a way that plausibly matters: a low-pole marker is
    10|either a real identity the model can inhabit ("predictable", "socially
...
is a property of the model or the items, and the ladder's reachable range has to
    20|be reported as a measured limit rather than treated as a bug.
...
def is_prefix_negation(low: str, high: str) -> bool:
    ...
    50|    Compared on the final word so multiword markers work: "artistically
...
def main(argv: list[str] | None = None) -> int:
    ...
    80|    from app.persona.intensity_prompts import (
        FACET_MARKERS,
        LEVEL_QUALIFIERS,
        TRAIT_MARKERS,
   ...
   100|    items = items_from_csv(Path(args.items_csv))
   ...
   110|        for level in levels:
            ...
            # Rotations repeat once variant*n_markers wraps the marker list.
   ...
   130|                    trait, level, variant=variant, n_markers=args.n_markers
                )
                start = (variant * args.n_markers) % len(markers)
                doubled = list(markers) + list(markers)
   ...
   140|                    model, tokenizer, dev, system, items, option_ids=option_ids
                )
                ...
                # Keyed per-item scores for the target trait, so a refusal can be
                # attributed to the items that would not move.
                per_item = []
                   150|                for i in target_idx:
                    raw = log["evs"][i]
                    if raw is None:
                        continue
                    per_item.append(
                        {
                            "text": items[i].text,
                            "keyed": items[i].keyed,
                            "keyed_score": round(reverse_scored_ev(raw, items[i].keyed), 3),
                        }
                    )
   ...
   170|                    "response_validity": round(response_validity(responses), 4),
                    "lock": lock,
                ...
                logger.info(
                    ...
                    trait, level, variant, n_negated, args.n_markers,
   180|                    rec["target_ev"], desc,
                )
   ...
   200|                )
    ...
    print("=" * 104)
    print(f"{'trait':18} {'lvl':>3} {'var':>3} {'neg':>4} {'ev':>6} {'argmax':>7} {'lock':>5}  markers")
    print("-" * 104)
    for r in rows:
        print(
            f"{r['trait']:18} {r['level']:>3} {r['variant']:>3} "
   210|            f"{r['n_negated_markers']:>4} {r['target_ev']:>6.2f} {r['target_argmax']:>7.2f} "
